...
grid = RectilinearGrid(; size = (Nx, Ny, Nz), halo=(3, 3, 3),
                       x = (-Lx/2, Lx/2),
                       y = (-Ly/2, Lx/2),
                       z = z_stretched,topology = (Bounded, Bounded, Bounded))
...
particles = LagrangianParticles(x=x₀, y=y₀, z=z₀, restitution=restitution)
...
simulation.output_writers[:particles] = 
                    NetCDFOutputWriter(model, model.particles, filename=string("my-tests/lagrangian-particles/particles_restitution=",restitution,".nc"), schedule=IterationInterval(10),
                    overwrite_existing=true)
simulation.output_writers[:buoyancy] = 
                NetCDFOutputWriter(model, (b=b,), filename=string("my-tests/lagrangian-particles/b_restitution=",restitution,".nc"), schedule=IterationInterval(10),
                overwrite_existing=true)
...
# Load particle data
fname = "my-tests/lagrangian-particles/particles_restitution=$restitution.nc"
...
# Load buoyancy data
fname = "my-tests/lagrangian-particles/b_restitution=$restitution.nc"
ds = Dataset(fname,"r")
...
frames = 1:length(t)
filename = "my-tests/lagrangian-particles/particles_animation_restitution=$restitution"
