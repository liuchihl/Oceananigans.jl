...
ω₀ = 1.4e-4     # tidal freq.
tᶠ = 300*2π/ω₀    # endtime of the simulation
θ = 0#3.6e-3      # slope angle
...
# Creates a vertical grid with near-ant spacing `refinement * Lz / Nz` near the bottom:
# "Warped" coordinate
kwarp(k, N) = (N + 1 - k) / N
...
coriolis = antCartesianCoriolis(f = f₀, rotation_axis = ĝ)

# Linear background stratification (in ẑ)
@inline ẑ(x, z, ĝ) = x*ĝ[1] .+ z*ĝ[3]
@inline ant_stratification(x, z, t, p) = p.N² * ẑ(x, z, p.ĝ)
B̄_field = BackgroundField(ant_stratification, parameters=(; ĝ, N² = N^2))
